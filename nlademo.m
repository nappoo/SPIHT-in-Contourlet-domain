...
% Input image
if ~exist('im', 'var') %检查im和var是否存在，不存在则执行if语句
    im = imread('barbara.png'); %读取图像barbara赋给im
    im = double(im) / 256; %将256个灰度级归一化
end

disp( 'Displaying the input image...');%显示输入图像
clf;%清除当前图窗窗口
imagesc(im, [0, 1]);%imagesc(___,clims) 指定映射到颜色图的第一个和最后一个元素的数据值。将 clims 指定为 [cmin cmax] 形式的二元素向量，其中小于或等于 cmin 的值映射到颜色图中的第一种颜色，大于或等于 cmax 的值映射到颜色图中的最后一种颜色。
title( 'Input image' ) ;
axis image off;
colormap(gray);
